...
function Metronome() {
  ...
  this.getSignatureCounter = function (n) {
    return signatureCounter;
  };
  this.getSignatureLength = function (n) {
  ...
  this.getNextPlayingNote = function () {
    return findNextPlayingNote();
  };
  this.getTempo = function () {
    return 60 / bpm;
  };
  this.getAccentAtBeat = function (n) {
    return getAccentAtBeatIndex(n - 1);
  };
...
function MetronomeUI(metronome) {
  ...
  function init() {
    $metronomeContainer.empty().append(initMetronome());

    $beatContainer.empty().append(initBeatBar());
  ...
  function initMetronome() {
    ...
    $base.css({
      ...
      'transform-origin': pxs(0, 0),
      transform: trns(0, opts.height - opts.width / 8, 0),
    });
    $left.css({
      position: 'absolute',
      width: px(opts.width / 2),
      height: px(opts.height * 2),
      'transform-origin': pxs(opts.width / 2, opts.height * 2),
      transform: trns(
    ...
    $right.css({
      ...
      width: px(opts.width / 2),
      height: px(opts.height * 2),
      'transform-origin': pxs(0, opts.height * 2),
      transform: trns(
  ...
  function initBeatBar() {
    ...
    for (var i = 0; i < m.getSignatureCounter(); i++) {
      ...
      let $beat = $('<div />').addClass('beat');
      let $accent = $('<div />').addClass('accent');
      $item.append($beat).append($accent).appendTo($bar);
  ...
  function onStopClick(e) {
    if (m.getIsPlaying()) {
      ...
      $beatContainer.find('.item.current').removeClass('current');
    }
  }
  ...
  function step(ts) {
    ...
    if (isPlaying == true) {
      ...
      if (totalBeatsPlayed === 0) {
        let currentTimeDelta = calcVariableDelta(
          0,
          0.5,
          startTime,
          currentTime,
        ...
        let currentAngleDelta =
          direction > 0 ? 0 + currentTimeDelta : 1 - currentTimeDelta;
      ...
      if (totalBeatsPlayed === 0) {
        let stopTimeDelta = calcVariableDelta(
          0,
          0.5,
          startTime,
          currentTime,
        ...
        let stopAngleDelta =
          direction > 0 ? 0 + stopTimeDelta : 1 - stopTimeDelta;
  ...
  function updateMetronomeArm(angle) {
    $rod.css({
      'transform-origin': pxs(opts.rodWidth / 2, opts.height * 2),
      transform: trns(
